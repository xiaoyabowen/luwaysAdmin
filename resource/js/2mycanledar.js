...
/*
showCanledarStart(changedMonth, initfirstday, dnow, daynumber,changedYear);
...
function commChanged(changedYear,changedMonth) {
    ...
    showCanledarStart(changedMonth, firstweekday, 1, allday,changedYear);
}
function showCanledarStart(month, firstday, dnow, allday,changedYear) {
    var dataJson = {};
    dataJson.skuId = getQueryParam("skuId").skuId;
    dataJson.startTime = changedYear + "-" + (month+1) + "-" + dnow;
    dataJson.endTime = changedYear + "-" + (month+1) + "-" + allday;
    ...
    for (var i = 0; i < rows; i++) {
        ...
        for (var k = 0; k < 7; k++) { // 表格每行的单元格
            ...
            (date_str <= 0 || date_str > allday) ?  tb+="<td style='background:#fff'> </td>" : tb += '<td class="tdActive">\n' +
                '                    <div class=" normal">\n' +
                '<input type="hidden" class="hideDayActive">'+
                '                        <em class="nowDateDay">'+date_str+'</em> ' +
...
$(".signincalendar").on("click","td.tdActive",function(){
    ...
    if ($(this).children(".normal").is(".activeDiv")){
        ...
        }else if ($("td.tdActive").children(".activeDiv").length > 1) {
            $(".peopleInput").val("");
            $(".numInput").val("");
            $(".rightContent").show()
        }
